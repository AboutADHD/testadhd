...
<!-- Vendored 2026-06-23 from https://github.com/aref-vc/tufte-claude-skill (MIT); copied into testadhd.ro 2026-06-24. The only quantitative surfaces here are the score bars / results (components/test/ScoreBar.tsx, Results.tsx) — there is no chart lib, so apply the principles with hand-rolled SVG/Tailwind; add Recharts only if a real need appears. The body's reference table lists presets/* helper files that were not vendored upstream; the inline principles stand on their own. -->
